...
if text.count(old_expected) != 1:
    raise SystemExit('first expected diagnostics')
...
old2 = """    await _dragWithWait(
    ...
    await tester.pump(const Duration(milliseconds: 250));
"""
new2 = """    await _pumpRealAsyncUntil(tester, () => tm.recordingTouchedTiles == null,
...
Future<void> _waitForAnyCanvasDifference(
  ...
  await _pumpRealAsyncUntil(tester, differs,
      timeout: const Duration(seconds: 3));
  final now = _readCanvas(tm, layer, w, h);
...
void _printCanvasDiagnostics(
  ...
  print('B20 $label: occupied=$opaqueA bbox=[$minAX,$minAY]-[$maxAX,$maxAY] diff=$diff diffBbox=[$minDX,$minDY]-[$maxDX,$maxDY] alphaOld=${alphaAt(20,19)} alphaMovedFar=${alphaAt(57,47)}');
}
...
Future<void> _waitForPixelAlpha(
  ...
  await _pumpRealAsyncUntil(tester, matches,
      timeout: const Duration(seconds: 3));
}
...
print('patched Batch20 with real floating-selection decode waits')
