...
Path('test/chain_vegetation_presets_test.dart').write_text(r'''import 'dart:ui' as ui;
...
void main() {
  ...
  test('chain and ball-chain presets are installed with distinct practical scales', () async {
    ...
    expect([thick.name, medium.name, thin.name, ball.name], [
      'チェーン（太）',
      'チェーン（中）',
      'チェーン（細）',
      'ボールチェーン',
  ...
  test('procedural chain renders repeated hollow links instead of a solid stroke', () async {
    final service = BrushService();
    await service.init();
  ...
  test('leaf and grass background stamps are multicolor dense procedural textures', () async {
    ...
    for (final stamp in [leaf, grass]) {
      ...
      for (var i = 0; i < texture.length; i += 4) {
        ...
        nonTransparent++;
        colors.add((texture[i] << 16) | (texture[i + 1] << 8) | texture[i + 2]);
      }
      expect(nonTransparent, greaterThan(900));
      expect(colors.length, greaterThanOrEqualTo(4), reason: '${stamp.name}に複数の緑色があること');
    }
  });

  test('existing-user stores receive the new built-ins without losing old items', () async {
    final seedBrushes = BrushService();
    await seedBrushes.init();
    final oldBrushJson = (await SharedPreferences.getInstance()).getStringList('brushes')!;
    final oldOnly = oldBrushJson.where((s) => !s.contains('Brush0018') && !s.contains('Brush0019') && !s.contains('Brush0020') && !s.contains('Brush0021')).toList();
    ...
    final oldStampJson = (await SharedPreferences.getInstance()).getStringList('stamps')!
...
''')
print('chain vegetation tests generated')
