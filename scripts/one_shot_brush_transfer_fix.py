...
# Named record passed through compute().
old = """      double scatter,\n      double density,\n    }) args) {"""
new = """      double scatter,\n      double density,\n      int opacity,\n    }) args) {"""
if text.count(old) != 1:
...
# Canvas -> isolate wiring. Some historical revisions contained the commit path
# twice; patch every real stamp call but fail for any other count.
replace_all_checked(
...
# ---------------------------------------------------------------------------
# Brush settings UI: Model + Engine are already implemented/tested, expose the
# three settings so the user can actually configure them.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# .niabrush import: do not reconstruct a subset of Brush fields. Rehydrate the
# complete model JSON, while replacing only device-specific id/folder/image path.
# ---------------------------------------------------------------------------
regex_once(
...
# ---------------------------------------------------------------------------
# Base .niatra serializer: the main transfer screen now adds binary assets via
# NiatraAssetBundle, but direct/legacy serializer use must not lose new fields.
# Use each model's JSON contract instead of maintaining partial parallel schemas.
# ---------------------------------------------------------------------------
