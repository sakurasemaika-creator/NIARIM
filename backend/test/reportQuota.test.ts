...
describe('通報レート制限の窓ID', () => {
  ...
  it('窓の長さは定数どおり10分', () => {
    ...
    const justInside = new Date(base.getTime() + (REPORT_WINDOW_MINUTES * 60 * 1000 - 1));
    const justOutside = new Date(base.getTime() + REPORT_WINDOW_MINUTES * 60 * 1000);
    expect(reportWindowId(base)).toBe(reportWindowId(justInside));
    expect(reportWindowId(base)).not.toBe(reportWindowId(justOutside));
  });

  it('日次窓はUTCの日付で切り替わる', () => {
    expect(reportDayWindowId(new Date('2026-09-02T23:59:59Z'))).toBe('D20260902');
    expect(reportDayWindowId(new Date('2026-09-03T00:00:00Z'))).toBe('D20260903');
  });
