...
describe('currentWindowId', () => {
  ...
  it('週間はISO週（月曜始まり）で、同じ週の月曜と日曜が同一IDになる', () => {
    ...
    expect(currentWindowId('weekly', monday)).toBe(currentWindowId('weekly', sunday));
    expect(currentWindowId('weekly', nextMonday)).not.toBe(currentWindowId('weekly', monday));
  });

  it('ISO週は年をまたぐ週で木曜の属する年を採る', () => {
    // 2027-01-01は金曜。その週の木曜は2026-12-31なので2026年の最終週。
    expect(isoWeekId(new Date('2027-01-01T00:00:00Z')).startsWith('2026')).toBe(true);
  });
});
