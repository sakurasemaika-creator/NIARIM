...
export async function toggleFollow(
  ...
  if (auth.niarimUserId === targetId)
    badRequest("自分自身をフォローすることはできません");

  const followerKey = Keys.followerRecord(targetId, auth.niarimUserId);
  const existing = await ddb.send(
    new GetCommand({ TableName: tableName(), Key: followerKey }),
  );
        ...
        new TransactWriteCommand({
          TransactItems: [
            ...
            },
            {
              Delete: {
                TableName: tableName(),
                Key: Keys.followingRecord(auth.niarimUserId, targetId),
              },
            },
            {
              Update: {
...
export async function getFollowers(
  ...
  const visibleFollowers = followerUsers
    ...
    .map((u) => ({
      niarimUserId: u.niarimUserId,
      channelName: u.channelName,
      channelAvatarUrl: u.channelAvatarUrl,
    }));
...
export async function getFollowing(
  ...
  const targets = await Promise.all(
    records.map((r) =>
      ddb.send(
        new GetCommand({ TableName: tableName(), Key: Keys.user(r.targetId) }),
      ),
    ),
  );
...
async function ensureCanViewList(
  callerId: string | undefined,
  targetId: string,
): Promise<void> {
  if (callerId === targetId) return;
  const target = await getUser(targetId);
  if (!target?.followersPublic)
    forbidden("このユーザーはフォロー中/フォロワー一覧を公開していません");
}

/** `PATCH /users/{id}/follow-visibility`（22.5節）。本人のみ変更可。 */
export async function updateFollowVisibility(
  event: APIGatewayProxyEventV2,
  targetUserId: string,
) {
  const auth = await authenticate(
