...
これらの基準は常時拘束するが、**毎セッション全文を再読しない**。開始時はAGENTS→continuation/checkpointを最優先で読み、仕様/品質/法務文書は今回の未完了項目に関係する節だけ検索・参照する。前回checkpointが「当該基準を確認済み」で、その後その基準/対象実装が変わっていなければ再読しない。コード変更前にCLAUDE/31番ガイドを対象機能・ファイル・ツール名で検索し、該当箇所だけ確認する。具体的な禁止・技術制約・検証条件は省略不可。
