...
void main() {
  ...
  test('眼鏡断層: マスク無し・空マスク・strength 0 は完全無変更', () {
    ...
    expect(
      e.applyLensDistortion(Uint8List.fromList(src), w, h, 70, null),
      orderedEquals(src),
    );
    expect(
      e.applyLensDistortion(
    ...
    expect(
      e.applyLensDistortion(Uint8List.fromList(src), w, h, 0, mask),
      orderedEquals(src),
    );
  });
  ...
  test('眼鏡断層: 選択範囲だけ変形し範囲外は完全保持', () async {
    ...
    final got = FilterEngine().applyLensDistortion(
      Uint8List.fromList(src),
      w,
      h,
      78,
  ...
  test('眼鏡断層: 正負度数が異なる視覚結果を生成', () async {
    const w = 200, h = 140;
    final src = _grid(w, h),
        mask = _ellipseMask(w, h, 100, 70, 62, 48),
        e = FilterEngine();
    final plus = e.applyLensDistortion(Uint8List.fromList(src), w, h, 82, mask);
    final minus = e.applyLensDistortion(
      Uint8List.fromList(src),
      w,
      h,
      -82,
  ...
  test('眼鏡断層: 中心オフセット X/Y は各々出力を変える', () async {
    ...
    final xm = e.applyLensDistortion(
      Uint8List.fromList(src),
      w,
      h,
      75,
    ...
    final yp = e.applyLensDistortion(
      Uint8List.fromList(src),
      w,
      h,
      75,
    ...
    final ym = e.applyLensDistortion(
      Uint8List.fromList(src),
      w,
      h,
      75,
    ...
    for (final v in [xp, xm, yp, ym]) {
      expect(_mad(base, v), greaterThan(.5));
    }
  ...
  test('眼鏡断層: 2連結成分を独立処理し中央の未選択帯は不変', () async {
    const w = 240, h = 150;
    final src = _grid(w, h),
        mask = _twoSeparatedMasks(w, h),
        e = FilterEngine();
    // 中央帯が本当に未選択であることを先に証明。
    for (var y = 45; y < 110; y++) {
      for (var x = 106; x < 134; x++) {
        expect(mask[(y * w + x) * 4 + 3], 0);
      }
    }
  ...
  test('オーロラホログラム: 6プリセットが固有出力・alpha保持', () async {
    ...
    for (final p in AuroraHologramPreset.values) {
      final got = e.applyAuroraHologram(
        Uint8List.fromList(src),
        w,
        h,
        strength: 100,
    ...
    for (var i = 0; i < AuroraHologramPreset.values.length; i++) {
      for (var j = i + 1; j < AuroraHologramPreset.values.length; j++) {
        expect(
          _mad(
            outputs[AuroraHologramPreset.values[i]]!,
            outputs[AuroraHologramPreset.values[j]]!,
          ),
          greaterThan(1),
        );
      }
...
Uint8List _grid(int w, int h) {
  ...
  for (var y = 0; y < h; y++) {
    for (var x = 0; x < w; x++) {
      final i = (y * w + x) * 4, l = x % 14 < 2 || y % 14 < 2;
      o[i] = l ? 30 : x * 255 ~/ math.max(1, w - 1);
      o[i + 1] = l ? 40 : y * 255 ~/ math.max(1, h - 1);
      o[i + 2] = l ? 55 : (x + y) * 255 ~/ math.max(1, w + h - 2);
      o[i + 3] = 255;
    }
  }
  return o;
...
Uint8List _twoSeparatedMasks(int w, int h) {
  final a = _ellipseMask(w, h, w * .27, h * .52, w * .15, h * .27),
      b = _ellipseMask(w, h, w * .73, h * .52, w * .15, h * .27);
  for (var i = 0; i < a.length; i += 4) {
    if (b[i + 3] > 0) {
      a[i] = a[i + 1] = a[i + 2] = a[i + 3] = 255;
    }
  }
  return a;
}

Uint8List _ramp(int w, int h) {
  final o = Uint8List(w * h * 4);
  for (var y = 0; y < h; y++) {
    for (var x = 0; x < w; x++) {
...
Future<void> _save(Uint8List rgba, int w, int h, String p) async {
  final c = Completer<ui.Image>();
  ui.decodeImageFromPixels(rgba, w, h, ui.PixelFormat.rgba8888, c.complete);
  final im = await c.future,
      d = await im.toByteData(format: ui.ImageByteFormat.png);
  im.dispose();
