...
void main() {
  ...
  test('1/5縮小＋複数斜め角度でも拡張描画範囲と書き出し枠は同じ変換へ追従する', () {
    ...
    for (final degrees in <double>[30, 45, 60, 90, -30, -45, -60, -90, 135]) {
      ...
      expect(transformedDrawing.contains(transformedExport.center), isTrue,
    ...
    for (final degrees in <double>[30, 45, 60, -30, -45, -60]) {
      ...
      for (final delta in <Offset>[
        ...
        expect(bounds.left, greaterThanOrEqualTo(-0.001),
            reason: '$degrees° $delta left');
        expect(bounds.top, greaterThanOrEqualTo(-0.001),
            reason: '$degrees° $delta top');
        expect(bounds.right, lessThanOrEqualTo(viewport.width + 0.001),
            reason: '$degrees° $delta right');
        expect(bounds.bottom, lessThanOrEqualTo(viewport.height + 0.001),
            reason: '$degrees° $delta bottom');
      }
    }
  });
}
