...
void main() {
  ...
  testWidgets('実CanvasAreaのマジックワンドで同色の非連結領域を分離選択し、タップ側だけ移動する', (tester) async {
    ...
    final p = (await tester.runAsync(() => projects.createProject(
      name: 'magicwand-functional', fps: 24, durationSeconds: 1,
    ...
    await tester.pumpWidget(MultiProvider(
      ...
      child: MaterialApp(home: Scaffold(body: Center(child: SizedBox(
        ...
        child: RepaintBoundary(
          key: boundaryKey,
          child: CanvasArea(project: p, currentLayerId: layer.id, currentTool: DrawingTool.selectMagicWand,
            currentFrame: 0, sceneId: scene.id, onSelectionActiveChanged: (v) => active = v),
        ),
      )))),
    ));
    await tester.pump(const Duration(seconds: 1));
    await tester.runAsync(() => _shot(boundaryKey, '${out.path}/selection_magicwand_00_before.png'));
    final origin = tester.getTopLeft(find.byType(CanvasArea));
    Offset at(double x, double y) => origin + Offset(x * 3, y * 3);

    final tap = await tester.startGesture(at(30, 33), kind: PointerDeviceKind.touch);
    await tester.pump();
    await tap.up(); await tester.pump();
    await _waitUntil(tester, () => active);
    await tester.runAsync(() => _shot(boundaryKey, '${out.path}/selection_magicwand_01_selected.png'));

    // 選択された左領域内を掴み、0,+22移動。
    final move = await tester.startGesture(at(30, 33), kind: PointerDeviceKind.touch);
    await tester.pump();
    expect(tm.recordingTouchedTiles, isNotNull, reason: 'ワンド選択後に選択変形へ入れること');
    await _waitAlpha(tester, tm, key, 30, 33, 0);
    await tester.runAsync(() => Future<void>.delayed(const Duration(milliseconds: 150))); await tester.pump();
    await move.moveTo(at(30, 55)); await tester.pump(const Duration(milliseconds: 40));
    await move.up(); await tester.pump();
    await _waitUntil(tester, () => undo.undoCount == 1);
    await tester.runAsync(() => _shot(boundaryKey, '${out.path}/selection_magicwand_02_after.png'));

    final actual = _read(tm, key, 96, 80);
    final expected = Uint8List.fromList(before);
    for (var y = 25; y < 41; y++) for (var x = 22; x < 38; x++) {
      final si=(y*96+x)*4, di=((y+22)*96+x)*4;
      expected.setRange(di,di+4,before,si); expected.fillRange(si,si+4,0);
    }
    ...
    await tester.runAsync(() => _shot(boundaryKey, '${out.path}/selection_magicwand_03_undo.png'));
    undo.redo(); await tester.pump(const Duration(milliseconds: 100));
    expect(_read(tm,key,96,80), orderedEquals(actual));
    await tester.runAsync(() => _shot(boundaryKey, '${out.path}/selection_magicwand_04_redo.png'));
  });
}
...
Future<void> _waitUntil(WidgetTester t,bool Function() f)async{final e=DateTime.now().add(const Duration(seconds:3));while(DateTime.now().isBefore(e)&&!f()){await t.runAsync(()=>Future<void>.delayed(const Duration(milliseconds:10)));await t.pump();}expect(f(),isTrue);}
Future<void> _waitAlpha(WidgetTester t,dynamic tm,String key,int x,int y,int a)async{await _waitUntil(t,(){final tile=tm.getTile(key,0,0) as Uint8List?;return (tile==null?0:tile[(y*256+x)*4+3])==a;});}
