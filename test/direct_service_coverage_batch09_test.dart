...
void main() {
  ...
  Future<Uint8List> bundledFontBytes() async {
    ...
    final path = manifest
        .listAssets()
        .firstWhere((p) => p.contains('assets/fonts/HakkouMincho.ttf'));
    final data = await rootBundle.load(path);
    return data.buffer.asUint8List(data.offsetInBytes, data.lengthInBytes);
  }

  test('real font add/read/rename/favorite/pixel/persist/remove works', () async {
    final service = FontService();
    await service.init();
    expect(service.catalog, isNotEmpty,
  ...
  test('bundled import preserves id, prevents duplicate and advances generated id', () async {
    final bytes = await bundledFontBytes();
    final service = FontService();
    await service.init();
    ...
    expect(service.isCatalogFontDownloaded(catalogEntry),
        service.fonts.any((f) => f.id == catalogEntry.id));
  }, timeout: const Timeout(Duration(seconds: 120)));

  test('corrupt ttf throws FontCorruptedException and cleans copied file', () async {
    final service = FontService();
    await service.init();
    ...
    expect(fontsDir.existsSync() ? fontsDir.listSync().whereType<File>() : const [],
        isEmpty);
  });
}
