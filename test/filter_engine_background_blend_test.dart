...
void main() {
  ...
  group('applyBackgroundBlend', () {
    ...
    test('透明画素（アルファ0）は変化しない', () {
      ...
      final result = engine.applyBackgroundBlend(data, width, height, blendColor, 0, 3, 0);
    ...
    test('向きを180°反転すると、光と影の側も入れ替わる（連動）', () {
      final data = buildSquare();
      final at0 = engine.applyBackgroundBlend(data, width, height, blendColor, 0, 3, 0);
      final at180 = engine.applyBackgroundBlend(data, width, height, blendColor, 180, 3, 0);
    ...
    test('lengthを大きくするほど影響範囲が広がる', () {
      final data = buildSquare();
      final shortLength = engine.applyBackgroundBlend(data, width, height, blendColor, 0, 1, 0);
      final longLength = engine.applyBackgroundBlend(data, width, height, blendColor, 0, 8, 0);
