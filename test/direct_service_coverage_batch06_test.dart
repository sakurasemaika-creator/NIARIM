...
void main() {
  ...
  test('built-ins apply preview commit save favorite reorder delete and persistence work', () async {
    ...
    await Future<void>.delayed(Duration.zero);
    expect(service.presets.firstWhere((p) => p.id == 'default_dark').name,
        'Preview Dark');
    ...
    await Future<void>.delayed(Duration.zero);
    expect(service.presets.firstWhere((p) => p.id == custom.id).isFavorite, isTrue);
    ...
    expect(restored.presets.firstWhere((p) => p.id == 'default_dark').name,
        'Preview Dark');
  ...
  test('saved subset is merged with missing built-ins and invalid current id falls back', () async {
    ...
    expect(service.presets.any((p) => p.id == 'default_light'), isTrue,
        reason: 'new/missing built-ins are merged into old saved lists');
    expect(service.current.id, 'only_custom',
