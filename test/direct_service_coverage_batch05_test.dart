...
void main() {
  ...
  test('default samples, find/add/update/remove and persistence work', () async {
    final service = AutofillPresetService();
    await service.init();
    expect(service.presets.map((p) => p.id), containsAll(['p1', 'p2']));
    expect(service.presets.firstWhere((p) => p.id == 'p1').parts.length,
        greaterThan(20));
    ...
    await service.updatePreset(custom.copyWith(name: 'Updated', isFavorite: true));
    ...
    await restored.init();
    expect(restored.presets.firstWhere((p) => p.id == custom.id).name, 'Updated');
  ...
  test('thumbnail replacement deletes old file and clear removes current file', () async {
    final service = AutofillPresetService();
    await service.init();
    ...
    final firstPath = service.presets.firstWhere((p) => p.id == presetId).thumbnailPath;
    ...
    final secondPath = service.presets.firstWhere((p) => p.id == presetId).thumbnailPath;
    ...
    expect(service.presets.firstWhere((p) => p.id == presetId).thumbnailPath, isNull);
  ...
  test('init repairs duplicate preset and part ids and upgrades untouched old samples', () async {
    ...
    const duplicateP1 = AutofillPreset(
      ...
      name: 'duplicate',
      parts: [
        AutofillPart(id: 'x', name: 'X', color: 0xFF333333),
      ],
    );
    SharedPreferences.setMockInitialValues({
      'autofill_presets': [
    ...
    final decodedIds = raw
        .map((s) => AutofillPreset.fromJson(jsonDecode(s) as Map<String, dynamic>).id)
        .toList();
