...
void main() {
  ...
  test('全17ブレンド：レイヤー不透明度50%でも標準合成式と一致', () async {
    for (final mode in LayerBlendMode.values) {
      ...
      final image = await LayerCompositor.composite(
        tm,
        [
          Layer(id: 'top', name: 'top', type: LayerType.normal, blendMode: mode, opacity: 50),
  ...
  test('レイヤー不透明度0/50/100が見た目へ線形に反映', () async {
    ...
    for (final opacity in [0, 50, 100]) {
      ...
      final image = await LayerCompositor.composite(
        tm,
        [
          Layer(id: 'top', name: 'top', type: LayerType.normal, opacity: opacity),
          const Layer(id: 'bottom', name: 'bottom', type: LayerType.normal),
        ],
        (l) => l.id, w, h,
      );
  ...
  test('クリッピング：上レイヤーは直下の元レイヤーalpha範囲内だけ表示', () async {
    ...
    for (var y = 30; y < 66; y++) {
      for (var x = 28; x < 68; x++) tm.setPixel(baseTile, x, y, 40, 160, 70, 255);
    }
    tm.markDirty('clipBase', 0, 0);
    final topTile = tm.getOrCreateTile('clipTop', 0, 0);
    for (var y = 10; y < 86; y++) {
      for (var x = 10; x < 86; x++) tm.setPixel(topTile, x, y, 230, 50, 80, 255);
    }
    ...
    final image = await LayerCompositor.composite(
      tm,
      const [
        Layer(id: 'clipTop', name: 'top', type: LayerType.normal, hasClipping: true),
    ...
    expect(_pixel(rgba, 48, 48).sublist(0, 3), equals([230, 50, 80]), reason: 'inside clip source gets top color');
    expect(_pixel(rgba, 15, 15)[3], 0, reason: 'top outside clip source must not show');
    image.dispose();
    tm.dispose();
  });
  ...
  test('縁取り描画フィルター：透明背景の図形の外周だけをリング化', () async {
    ...
    for (var y = 30; y < 66; y++) {
      for (var x = 30; x < 66; x++) {
        final i = (y * w + x) * 4;
        input[i] = 40; input[i + 1] = 120; input[i + 2] = 220; input[i + 3] = 255;
      }
    }
    final result = applyDrawFilterInIsolate((
      input, w, h,
      const FilterDef(
        id: 'outline-shape', name: 'outline-shape', kind: FilterKind.outline,
        outlineColor: 0xFFFF3040, outlineWidth: 4,
      ),
      null,
    ));
    ...
    expect(_pixel(result, 48, 48)[3], 0, reason: 'outline layer must not contain original interior');
    final justOutside = _pixel(result, 28, 48);
    expect(justOutside[3], greaterThan(0), reason: 'ring must exist just outside shape');
    expect(justOutside[0], greaterThan(200));
    expect(_pixel(result, 10, 10)[3], 0, reason: 'far background must stay transparent');
  });
}
...
List<double> _clip(List<double> c) {
  ...
  if(n<0){r=l+(r-l)*l/(l-n);g=l+(g-l)*l/(l-n);b=l+(b-l)*l/(l-n);} final x=math.max(r,math.max(g,b));
  if(x>1){r=l+(r-l)*(1-l)/(x-l);g=l+(g-l)*(1-l)/(x-l);b=l+(b-l)*(1-l)/(x-l);} return [r,g,b];
}
List<double> _setLum(List<double> c,double l){final d=l-_lum(c);return _clip([c[0]+d,c[1]+d,c[2]+d]);}
List<double> _setSat(List<double> c,double s){final idx=[0,1,2]..sort((a,b)=>c[a].compareTo(c[b]));final o=List<double>.from(c);final lo=idx[0],mi=idx[1],hi=idx[2];if(c[hi]>c[lo]){o[mi]=(c[mi]-c[lo])*s/(c[hi]-c[lo]);o[hi]=s;}else{o[mi]=0;o[hi]=0;}o[lo]=0;return o;}
