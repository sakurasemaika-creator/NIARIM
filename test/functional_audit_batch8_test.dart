...
void main() {
  ...
  test('ブレンドモード：複数の実色組み合わせを重ねて全17種のRGB変化を照合', () async {
    ...
    for (final mode in LayerBlendMode.values) {
      for (var i = 0; i < pairs.length; i++) {
        ...
        _fillLayer(tm, 'top', 64, 64, pairs[i].$2, left: 16, top: 16, right: 48, bottom: 48);
        ...
        for (var c = 0; c < 3; c++) {
          expect((overlap[c] - expected[c]).abs(), lessThanOrEqualTo(5),
              reason: '${mode.name} pair=$i channel=$c actual=${overlap[c]} expected=${expected[c]}');
        }
        expect(outside.sublist(0, 3), equals([pairs[i].$1.red, pairs[i].$1.green, pairs[i].$1.blue]),
            reason: '${mode.name} must not affect non-overlap area');
        if (i == 0) await _save(image, '${out.path}/blend_interaction_${mode.name}.png');
        image.dispose();
        tm.dispose();
      }
    }
  ...
  test('ブラシ不透明度：50%の線2本を交差させ非交差部50%、交差部75%になる', () async {
    ...
    expect(single[3], inInclusiveRange(124, 132),
        reason: 'one 50% stroke must remain at the configured opacity along its body');
    expect(cross[3], inInclusiveRange(188, 196),
  ...
  test('投げ縄：不規則多角形の外側へ1pxも塗りが漏れない', () async {
    ...
    const height = 96;
    const polygon = [
      ui.Offset(13, 19), ui.Offset(74, 14), ui.Offset(84, 47),
      ui.Offset(67, 82), ui.Offset(29, 74), ui.Offset(17, 52),
    ];
    ...
    await _saveRgba(result, width, height, '${out.path}/lasso_full_boundary_scan.png');
    ...
    expect(leaked, 0, reason: 'lasso fill must not paint any pixel whose center is outside the polygon');
  ...
  test('クリッピング：上レイヤー由来の色が基底alpha外へ1pxも出ない', () async {
    ...
    _fillLayer(tm, 'base', width, height, const ui.Color(0xFF208050), left: 26, top: 24, right: 70, bottom: 72);
    _fillLayer(tm, 'top', width, height, const ui.Color(0xFFE03060), left: 4, top: 4, right: 92, bottom: 92);

    final image = await LayerCompositor.composite(
      tm,
      const [
        Layer(id: 'top', name: 'top', type: LayerType.normal, hasClipping: true),
  ...
  test('ピクセルモード：ブラシサイズに比例してドット径が変わり整数グリッドがずれない', () async {
    ...
    expect(_normalizeMask(a.$1, a.$2), equals(_normalizeMask(b.$1, b.$2)),
        reason: 'pixel brush footprint must stay locked to the integer pixel grid');
...
Brush _brush({required double size, required int opacity, required int spacing}) => Brush(
  id: 'audit', name: 'audit', size: size, opacity: opacity, spacing: spacing,
...
Future<(Uint8List, int)> _pixelStampMask({required int canvas, required double size, required double cx, required double cy}) async {
  final tm = TileManager(canvasWidth: canvas, canvasHeight: canvas);
  final engine = DrawingEngine(tileManager: tm)
    ..currentBrush = Brush(
      id: 'px', name: 'px', size: size, opacity: 100, spacing: 1,
      blurRadius: 0, stabilization: false, stabilizationStrength: 0,
      pixelMode: true, pressureMode: PressureMode.off, pressureStrength: 100,
      fadeMode: FadeMode.off, strokeDecay: false,
      mixingMode: BrushMixingMode.off, mixingRate: 0,
    )
...
int _bboxWidth(Uint8List d, int width) {
  ...
  for (var y = 0; y < width; y++) for (var x = 0; x < width; x++) {
    if (d[(y * width + x) * 4 + 3] > 0) { minX = math.min(minX, x); maxX = math.max(maxX, x); }
  }
...
int _bboxHeight(Uint8List d, int width) {
  ...
  for (var y = 0; y < width; y++) for (var x = 0; x < width; x++) {
    if (d[(y * width + x) * 4 + 3] > 0) { minY = math.min(minY, y); maxY = math.max(maxY, y); }
  }
...
List<String> _normalizeMask(Uint8List d, int width) {
  ...
  for (var y = minY; y <= maxY; y++) {
    ...
    for (var x = minX; x <= maxX; x++) sb.write(d[(y * width + x) * 4 + 3] > 0 ? '1' : '0');
...
void _fillLayer(TileManager tm, String id, int width, int height, ui.Color color,
    {int left = 0, int top = 0, int? right, int? bottom}) {
  ...
  for (var y = top; y < b; y++) {
    for (var x = left; x < r; x++) {
      ...
      tm.setPixel(tile, x % TileManager.tileSize, y % TileManager.tileSize,
...
List<int> _blend(LayerBlendMode mode, ui.Color backdrop, ui.Color source) {
  ...
  List<double> o;
  switch (mode) {
    case LayerBlendMode.normal: o = cs;
    case LayerBlendMode.multiply: o = List.generate(3, (i) => cb[i] * cs[i]);
    case LayerBlendMode.screen: o = List.generate(3, (i) => cb[i] + cs[i] - cb[i] * cs[i]);
    case LayerBlendMode.overlay: o = List.generate(3, (i) => cb[i] <= .5 ? 2 * cb[i] * cs[i] : 1 - 2 * (1 - cb[i]) * (1 - cs[i]));
    case LayerBlendMode.addition: o = List.generate(3, (i) => math.min(1, cb[i] + cs[i]));
    case LayerBlendMode.subtract: o = List.generate(3, (i) => math.max(0, cb[i] - cs[i]));
    case LayerBlendMode.darken: o = List.generate(3, (i) => math.min(cb[i], cs[i]));
    case LayerBlendMode.lighten: o = List.generate(3, (i) => math.max(cb[i], cs[i]));
    case LayerBlendMode.colorBurn: o = List.generate(3, (i) => cs[i] <= 0 ? 0 : 1 - math.min(1, (1 - cb[i]) / cs[i]));
    case LayerBlendMode.colorDodge: o = List.generate(3, (i) => cs[i] >= 1 ? 1 : math.min(1, cb[i] / (1 - cs[i])));
    case LayerBlendMode.hardLight: o = List.generate(3, (i) => cs[i] <= .5 ? 2 * cb[i] * cs[i] : 1 - 2 * (1 - cb[i]) * (1 - cs[i]));
    ...
    case LayerBlendMode.difference: o = List.generate(3, (i) => (cb[i] - cs[i]).abs());
    case LayerBlendMode.hue: o = _setLum(_setSat(cs, _sat(cb)), _lum(cb));
    case LayerBlendMode.saturation: o = _setLum(_setSat(cb, _sat(cs)), _lum(cb));
    case LayerBlendMode.color: o = _setLum(cs, _lum(cb));
    case LayerBlendMode.luminosity: o = _setLum(cb, _lum(cs));
  }
...
List<double> _clip(List<double> c) {
  var r = c[0], g = c[1], b = c[2]; final l = _lum([r,g,b]); final n = math.min(r, math.min(g,b));
...
List<double> _setLum(List<double> c, double l) { final d=l-_lum(c); return _clip([c[0]+d,c[1]+d,c[2]+d]); }
List<double> _setSat(List<double> c, double s) {
  final idx=[0,1,2]..sort((a,b)=>c[a].compareTo(c[b])); final o=List<double>.from(c); final lo=idx[0],mi=idx[1],hi=idx[2];
  if(c[hi]>c[lo]){o[mi]=(c[mi]-c[lo])*s/(c[hi]-c[lo]);o[hi]=s;}else{o[mi]=0;o[hi]=0;}o[lo]=0;return o;
}
