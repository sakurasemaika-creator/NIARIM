...
void main() {
  test('canvas root wires brush outline eyedropper into composite sampling', () {
    ...
    expect(source, contains('pendingBrushOutline.complete(color.toARGB32())'));
    expect(source, isNot(contains('_pendingBrushOutlineEyedropper?.complete(color.toARGB32())')));
    expect(source, contains('_brushOutlineEyedropperActive'));
  });

  test('brush settings dismisses modal sheet before canvas outline sampling', () {
    final source = File('lib/screens/canvas/widgets/brush_panel.dart').readAsStringSync();

    final methodStart = source.indexOf('Future<void> _eyedropOutlineColor() async');
    ...
    expect(method, contains('service.updateBrush(draft.copyWith(outlineColor: sampled));'));
  });
}
