...
void main() {
  ...
  test('camera pan/zoom/rotation interpolates and produces distinct visual frames', () async {
    ...
    for (var frame = 0; frame <= 10; frame++) {
      ...
      rendered.add(rgba);
      await _saveImage(image, '${out.path}/camera_${frame.toString().padLeft(2, '0')}.png');
      image.dispose();
    }
  ...
  test('fade presentation effect changes continuously across its active range', () async {
    ...
    for (final frame in [0, 2, 4, 6, 8, 10]) {
      final got = engine.applyEffectFilters(Uint8List.fromList(src), w, h, const [effect], frame);
      frames[frame] = got;
      await _saveRgba(got, w, h, '${out.path}/effect_fade_${frame.toString().padLeft(2, '0')}.png');
    }
  ...
  test('animatedNoise presentation effect changes from frame to frame', () async {
    ...
    for (var frame = 0; frame <= 5; frame++) {
      final got = engine.applyEffectFilters(Uint8List.fromList(src), w, h, const [effect], frame);
      frames.add(got);
      await _saveRgba(got, w, h, '${out.path}/effect_animated_noise_${frame.toString().padLeft(2, '0')}.png');
    }
    for (var i = 1; i < frames.length; i++) {
      expect(_meanAbsDiff(frames[i - 1], frames[i]), greaterThan(1), reason: 'animatedNoise frame $i must move/change');
    }
  });
  ...
  test('rain presentation effect moves across consecutive frames', () async {
    ...
    for (var frame = 0; frame <= 5; frame++) {
      final got = engine.applyEffectFilters(Uint8List.fromList(src), w, h, const [effect], frame);
      frames.add(got);
      await _saveRgba(got, w, h, '${out.path}/effect_rain_${frame.toString().padLeft(2, '0')}.png');
    }
    for (var i = 1; i < frames.length; i++) {
      expect(_meanAbsDiff(frames[i - 1], frames[i]), greaterThan(0.15), reason: 'rain frame $i must move/change');
    }
  });
  ...
  test('presentation effects obey frame range and enabled state', () {
    ...
    expect(engine.applyEffectFilters(Uint8List.fromList(src), w, h, const [ranged], 2), orderedEquals(src));
    expect(engine.applyEffectFilters(Uint8List.fromList(src), w, h, const [ranged], 8), orderedEquals(src));
    expect(engine.applyEffectFilters(Uint8List.fromList(src), w, h, const [disabled], 5), orderedEquals(src));
    expect(_meanAbsDiff(src, engine.applyEffectFilters(Uint8List.fromList(src), w, h, const [ranged], 5)), greaterThan(0));
  });
}

Future<ui.Image> _renderCameraFrame(CameraEngine engine, CameraKeyframe kf, int w, int h) async {
  final recorder = ui.PictureRecorder();
  final canvas = ui.Canvas(recorder);
  canvas.drawRect(ui.Rect.fromLTWH(0, 0, w.toDouble(), h.toDouble()), ui.Paint()..color = const ui.Color(0xFFF4EEE4));
  canvas.save();
  engine.apply(canvas, kf, w.toDouble(), h.toDouble());
  canvas.drawRect(const ui.Rect.fromLTWH(22, 22, 62, 34), ui.Paint()..color = const ui.Color(0xFFE05252));
