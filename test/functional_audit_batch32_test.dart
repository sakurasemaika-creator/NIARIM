...
void main() {
  ...
  testWidgets('実CanvasAreaの垂直直線定規でDownからUpまで全ストロークがx=48付近へ拘束される', (tester) async {
    ...
    final undo = app_undo.UndoManager(); projects.setUndoManager(undo);
    final p = (await tester.runAsync(() => projects.createProject(
      name:'ruler-line-functional', fps:24, durationSeconds:1,
    ...
    const ruler=Ruler(
      type:RulerType.line, position:Offset(48,40), rotation:1.5707963267948966,
    ...
    await tester.pumpWidget(MultiProvider(
      ...
      child:MaterialApp(home:Scaffold(body:Center(child:SizedBox(width:288,height:240,
        child:CanvasArea(project:p,currentLayerId:layer.id,currentTool:DrawingTool.ruler,
          currentFrame:0,sceneId:scene.id,activeRuler:ruler),
      )))),
    ));
    ...
    var offGuide=0, onGuide=0;
    for(var y=10;y<75;y++)for(var x=0;x<96;x++){
      if(rgba[(y*96+x)*4+3]==0)continue;
      if((x-48).abs()<=5) onGuide++; else offGuide++;
    }
