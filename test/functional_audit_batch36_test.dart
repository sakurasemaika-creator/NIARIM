...
void main() {
  ...
  test('全7種類の定規を本番RulerEngine→DrawingEngineで実描画しPNGと幾何拘束を検査する', () async {
    ...
    await brushes.init();
    final baseBrush = brushes.brushes
        .firstWhere((b) => b.id == 'Brush0002')
        .copyWith(size: 3, opacity: 100);
    ...
    final cases =
        <
          ({
            ...
            Ruler ruler,
            List<ui.Offset> raw,
            double Function(double, double) distance,
          })
        >[
          (
            name: 'line',
            ruler: const Ruler(
              type: RulerType.line,
              position: ui.Offset(64, 64),
              rotation: math.pi / 2,
              settings: RulerSettings(),
            ),
            raw: [
              for (double y = 15; y <= 112; y += 8)
                ui.Offset(90 + math.sin(y) * 8, y),
            ],
          ...
          (
            ...
            ruler: const Ruler(
              ...
              position: ui.Offset(64, 64),
              settings: RulerSettings(radiusX: 36),
            ),
            raw: [
              for (int i = 0; i <= 24; i++)
                ui.Offset(
                  64 + 45 * math.cos(i * math.pi * 2 / 24),
                  64 + 28 * math.sin(i * math.pi * 2 / 24),
                ),
            ],
            distance: (x, y) =>
                ((ui.Offset(x, y) - const ui.Offset(64, 64)).distance - 36)
                    .abs(),
          ),
          (
            name: 'ellipse',
            ruler: const Ruler(
              ...
              position: ui.Offset(64, 64),
              rotation: 0.35,
              settings: RulerSettings(radiusX: 40, radiusY: 23),
            ),
            raw: [
              for (int i = 0; i <= 28; i++)
                ui.Offset(
                  64 + 48 * math.cos(i * math.pi * 2 / 28),
                  64 + 30 * math.sin(i * math.pi * 2 / 28),
                ),
            ],
            distance: (x, y) {
              ...
              return (((lx * lx) / (40 * 40) + (ly * ly) / (23 * 23)) - 1)
                      .abs() *
                  20;
            },
          ),
          (
            ...
            raw: [
              for (double r = 12; r <= 55; r += 4)
                ui.Offset(64 + r * math.cos(0.48), 64 + r * math.sin(0.48)),
            ],
            distance: (x, y) => _distanceToNearestSpoke(
              ui.Offset(x, y),
              const ui.Offset(64, 64),
              8,
              0,
            ),
          ),
          (
            ...
            ruler: const Ruler(
              ...
              position: ui.Offset(20, 20),
              settings: RulerSettings(vanishingPoint1: ui.Offset(20, 20)),
            ),
            raw: [
              for (double t = 0; t <= 1.0; t += 0.08)
                ui.Offset(
                  88 - 50 * t + 8 * math.sin(t * 15),
                  102 - 60 * t + 5 * math.cos(t * 11),
                ),
            ],
            distance: (x, y) => _distanceToInfiniteLine(
              ui.Offset(x, y),
              const ui.Offset(20, 20),
              const ui.Offset(88, 102),
            ),
          ),
          (
            name: 'perspective2',
            ruler: const Ruler(
              ...
              position: ui.Offset(64, 64),
              settings: RulerSettings(
                vanishingPoint1: ui.Offset(15, 35),
                vanishingPoint2: ui.Offset(113, 35),
              ),
            ),
            raw: [
              for (double t = 0; t <= 1.0; t += 0.08)
                ui.Offset(92 - 35 * t, 100 - 25 * t + 6 * math.sin(t * 12)),
            ],
            // First anchor(92,100) is nearer VP2=(113,35), so the full stroke must stay on that ray.
            distance: (x, y) => _distanceToInfiniteLine(
              ui.Offset(x, y),
              const ui.Offset(113, 35),
              const ui.Offset(92, 100),
            ),
          ),
          (
            name: 'perspective3',
            ruler: const Ruler(
              ...
              position: ui.Offset(64, 64),
              settings: RulerSettings(
                vanishingPoint1: ui.Offset(15, 40),
                vanishingPoint2: ui.Offset(113, 40),
                vanishingPoint3: ui.Offset(64, 8),
              ),
            ),
            raw: [
              for (double t = 0; t <= 1.0; t += 0.08)
                ui.Offset(70 + 12 * math.sin(t * 8), 92 - 35 * t),
            ],
            // Anchor (70,92) is nearest VP2=(113,40); RulerEngine chooses the nearest
            // vanishing point at stroke start and keeps that ray for the whole stroke.
            distance: (x, y) => _distanceToInfiniteLine(
              ui.Offset(x, y),
              const ui.Offset(113, 40),
              const ui.Offset(70, 92),
            ),
          ),
        ];
    ...
    for (final c in cases) {
      ...
      draw.beginStroke(
        StrokePoint(x: first.dx, y: first.dy, pressure: 1),
        'paint',
      );
      for (final p in c.raw.skip(1)) {
        draw.continueStroke(
          StrokePoint(x: p.dx, y: p.dy, pressure: 1),
          'paint',
        );
      }
      draw.endStroke();
      ...
      expect(
        painted,
        greaterThan(20),
        reason: '${c.name}: actual raster stroke must be visible',
      );
      expect(
        far,
        0,
        reason:
            '${c.name}: every rasterized brush pixel must remain within 5px of the ruler geometry (max=$maxDistance)',
      );
      img.dispose();
      tm.dispose();
    }
...
double _distanceToNearestSpoke(
  ui.Offset p,
  ui.Offset center,
  int divisions,
  double rotation,
