...
void main() {
  ...
  test('墨溜まり: 45°/90°には発生し、鈍角135°と直線には発生しない', () async {
    ...
    await _save(_composite(acute45, a45), w, h, '${out.path}/ink_angle45_after.png');
    await _save(right90, w, h, '${out.path}/ink_angle90_before.png');
    await _save(_composite(right90, a90), w, h, '${out.path}/ink_angle90_after.png');
    await _save(_composite(obtuse135, a135), w, h, '${out.path}/ink_angle135_no_effect.png');
    await _save(_composite(straight180, a180), w, h, '${out.path}/ink_straight_no_effect.png');
  });
  ...
  test('墨溜まり: 範囲と中央太さが独立して効き、指定色・端1pxテーパーを保つ', () async {
    ...
    // 中心付近は横断方向に複数px、範囲端近くは中心より細いことを数値確認。
    final cx = w ~/ 2, cy = h ~/ 2;
    ...
    await _save(_composite(src, narrow), w, h, '${out.path}/ink_range10_width5.png');
    await _save(_composite(src, longRange), w, h, '${out.path}/ink_range28_width5.png');
    await _save(_composite(src, thickCenter), w, h, '${out.path}/ink_range28_width14.png');
  });
}
...
Uint8List _animeFace(int w, int h) {
  ...
  void line(double x0, double y0, double x1, double y1, {int shade = 42, int thick = 2}) {
    final steps = math.max((x1 - x0).abs(), (y1 - y0).abs()).ceil();
    for (var s = 0; s <= steps; s++) {
      final t = steps == 0 ? 0.0 : s / steps;
      _dot(b, w, h, (x0 + (x1 - x0) * t).round(), (y0 + (y1 - y0) * t).round(), shade, thick);
    }
  }
  void ellipse(double cx, double cy, double rx, double ry, {int shade = 42, int thick = 2}) {
    for (var n = 0; n < 720; n++) {
      final a = 2 * math.pi * n / 720;
      _dot(b, w, h, (cx + rx * math.cos(a)).round(), (cy + ry * math.sin(a)).round(), shade, thick);
    }
  }
...
void _drawLine(Uint8List b, int w, int h, int x0, int y0, int x1, int y1) {
  ...
  for (var s = 0; s <= steps; s++) {
    final t = steps == 0 ? 0.0 : s / steps;
    _dot(b, w, h, (x0 + (x1 - x0) * t).round(), (y0 + (y1 - y0) * t).round(), 24, 2);
  }
}
...
void _fillEllipseMask(Uint8List b, int w, int h, double cx, double cy, double rx, double ry) {
  for (var y = 0; y < h; y++) {
    for (var x = 0; x < w; x++) {
...
int _verticalInkSpan(Uint8List b, int w, int h, int x) {
  var minY = h, maxY = -1;
  for (var y = 0; y < h; y++) {
...
double _regionMad(Uint8List a, Uint8List b, int w, int x0, int y0, int x1, int y1) {
  var s = 0.0, n = 0;
  for (var y = y0; y < y1; y++) {
    for (var x = x0; x < x1; x++) {
      final i = (y * w + x) * 4;
