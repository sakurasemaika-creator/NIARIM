...
void main() {
  ...
  test('メッシュ変形：規則格子が指定boundsを均等分割', () {
    final grid = MeshWarpEngine.regularGrid(2, 3, const ui.Rect.fromLTWH(10, 20, 60, 40));
  ...
  test('メッシュ変形：恒等格子では元画像をほぼ維持', () async {
    ...
    final grid = MeshWarpEngine.regularGrid(2, 2, const ui.Rect.fromLTWH(0, 0, 48, 48));
    ...
    for (final p in const [ui.Offset(8, 8), ui.Offset(24, 24), ui.Offset(39, 31)]) {
      final pa = _pixel(a, 48, p.dx.toInt(), p.dy.toInt());
      final pb = _pixel(b, 48, p.dx.toInt(), p.dy.toInt());
      for (var c = 0; c < 4; c++) {
        expect((pa[c] - pb[c]).abs(), lessThanOrEqualTo(8), reason: 'identity warp ${p.dx},${p.dy} c=$c');
      }
    }
    image.dispose();
    warped.dispose();
  ...
  test('メッシュ変形：制御点移動で図形が実際に歪む', () async {
    ...
    for (var y = 8; y < 40; y++) {
      for (var x = 8; x < 40; x++) {
        final i = (y * 48 + x) * 4;
        input[i] = 230; input[i + 1] = 70; input[i + 2] = 40; input[i + 3] = 255;
      }
    }
    final image = await _image(input, 48, 48);
    final grid = MeshWarpEngine.regularGrid(1, 1, const ui.Rect.fromLTWH(0, 0, 48, 48));
    final moved = List<ui.Offset>.from(grid);
    moved[1] = const ui.Offset(36, 8);
    moved[3] = const ui.Offset(46, 46);
    final warped = await MeshWarpEngine.warp(
      image: image, rows: 1, cols: 1, controlPoints: moved, outputWidth: 48, outputHeight: 48);
  ...
  test('定規：直線・円・楕円・放射のスナップ幾何が正しい', () {
    ...
    e.setActiveRuler(const Ruler(type: RulerType.line, position: ui.Offset(10, 10), rotation: 0, settings: RulerSettings()));
    expect(e.snapToRuler(const ui.Offset(30, 25)).dy, closeTo(10, 1e-9));

    e.setActiveRuler(const Ruler(type: RulerType.circle, position: ui.Offset(50, 50), settings: RulerSettings(radiusX: 20)));
    final c = e.snapToRuler(const ui.Offset(80, 50));
    expect(c.dx, closeTo(70, 1e-9));
    expect(c.dy, closeTo(50, 1e-9));

    e.setActiveRuler(const Ruler(type: RulerType.ellipse, position: ui.Offset(50, 50), settings: RulerSettings(radiusX: 30, radiusY: 10)));
    final ell = e.snapToRuler(const ui.Offset(50, 90));
    expect(ell.dx, closeTo(50, 1e-6));
    expect(ell.dy, closeTo(60, 1e-6));

    e.setActiveRuler(const Ruler(type: RulerType.radial, position: ui.Offset(50, 50), settings: RulerSettings(divisions: 4)));
  ...
  test('定規：透視定規は1ストローク中に同じ消失点直線へ固定', () {
    final e = RulerEngine();
    e.setActiveRuler(const Ruler(
      type: RulerType.twoPointPerspective,
      position: ui.Offset.zero,
      settings: RulerSettings(vanishingPoint1: ui.Offset(0, 50), vanishingPoint2: ui.Offset(100, 50)),
    ));
    e.beginStroke();
  ...
  test('スタンプ：透明背景へRGBAを正しくsrc-over合成', () async {
    ...
    final result = StampEngine().stampAlongPath(
      canvasData: canvas, width: w, height: h, texture: tex, texSize: 2,
      points: const [ui.Offset(48, 48)], stampSize: 20,
    );
  ...
  test('スタンプ：densityとscatterはseed固定で決定的', () {
    ...
    for (var i = 0; i < tex.length; i += 4) {
      tex[i] = 200; tex[i + 1] = 80; tex[i + 2] = 40; tex[i + 3] = 255;
    }
    final points = [for (var x = 10.0; x <= 86; x += 4) ui.Offset(x, 48)];
    final a = StampEngine().stampAlongPath(
      canvasData: Uint8List(w * h * 4), width: w, height: h, texture: tex, texSize: 4,
      points: points, stampSize: 8, scatter: 10, density: 0.55, seed: 42,
    );
