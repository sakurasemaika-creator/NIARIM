...
void main() {
  ...
  test('repeated discrete filter actions are preserved in recording order', () async {
    ...
    service.recordStep(
      ...
      label: '自動線画',
      args: const {'filter': {'id': 'Filter0023'}},
      recordedFrame: 0,
    );
    service.recordStep(
      surface: CustomAutomationSurface.canvas,
      command: 'canvas.filter',
      label: '墨溜まり',
      args: const {'filter': {'id': 'Filter0021'}},
      recordedFrame: 0,
    );
  ...
  test('continuous brush-size changes still coalesce to the latest value', () async {
    final service = CustomAutomationService();
    await service.init();
    service.beginDraft(
      name: 'brush',
