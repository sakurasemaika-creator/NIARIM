...
void main() {
  ...
  test(
    'stylus/touch/mouse classification and pressure/tilt conversion are correct',
    () {
  ...
  test(
    'material lifecycle persists, deduplicates, detects missing files and builds share bundle',
    () async {
      ...
      final share = await service.buildShareBundle(projectId, {
        MaterialType.image,
      });
  ...
  test(
    'current Premium local path stays store-free and campaign-enabled before monetization date',
    () async {
  ...
  test(
    'purchased Premium metadata restores locally without requiring store access',
    () async {
