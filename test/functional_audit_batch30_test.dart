...
void main() {
  ...
  testWidgets('実CanvasAreaの投げ縄選択で囲んだ赤図形だけを移動し外の青図形は全RGBA不変', (tester) async {
    ...
    final p = (await tester.runAsync(() => projects.createProject(
      name: 'lasso-selection-functional', fps: 24, durationSeconds: 1,
    ...
    // 不規則5角形で赤だけを囲う。
    final g = await tester.startGesture(at(18, 24), kind: PointerDeviceKind.touch);
    for (final pnt in const [Offset(48, 19), Offset(56, 44), Offset(36, 54), Offset(17, 43), Offset(18, 24)]) {
      await g.moveTo(at(pnt.dx, pnt.dy)); await tester.pump();
    }
    ...
    for (var y = 30; y < 39; y++) for (var x = 30; x < 39; x++) {
      final si = (y * 96 + x) * 4; final di = ((y + 10) * 96 + (x + 20)) * 4;
      expected.setRange(di, di + 4, before, si); expected.fillRange(si, si + 4, 0);
    }
    expect(actual, orderedEquals(expected), reason: '投げ縄外の青図形を含む全画素は変えず、囲った赤だけ移動すること');
