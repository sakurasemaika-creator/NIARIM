...
void main() {
  ...
  test('ピクセルモード連続線：長い斜線でも中間alpha無し・穴無し・同じ入力はキャンバス寸法に依存しない', () async {
    ...
    final b = await _drawPixelLine(canvas: 160, offset: const ui.Offset(24, 30));
    ...
    expect(alphaSet.every((v) => v == 0 || v == 255), isTrue,
    ...
    for (var i = 0; i <= 24; i++) {
      ...
      expect(_hasOpaqueNear(a, 96, x, y, radius: 2), isTrue, reason: 'hole at $x,$y');
    }
    ...
    for (var y = 8; y < 88; y++) {
      for (var x = 8; x < 88; x++) {
        ...
        expect(bb, aa, reason: 'pixel-grid footprint mismatch at relative $x,$y');
      }
    }
  });
  ...
  test('色伸び：青下地を赤ブラシで連続ストロークすると開始側ほど下地色を拾い後半で選択色へ寄る', () async {
    ...
    expect(late[0], greaterThan(early[0]), reason: 'later stroke should move toward selected red');
    expect(late[2], lessThan(early[2]), reason: 'later stroke should carry less picked-up blue');
    image.dispose();
    tm.dispose();
  });
  ...
  test('テキスト：横書き/縦書き/縁取り/pixelModeを実ラスタライズして形状とalphaを確認', () async {
    const base = TextObject(
      id: 't', text: 'NIARIM 12', fontSize: 28,
      color: ui.Color(0xFF202020), position: ui.Offset(20, 25),
    );
    ...
    final verticalObj = base.copyWith(
      text: '縦12A', direction: TextWritingDirection.vertical,
    ...
    expect(hb.width, greaterThan(hb.height), reason: 'horizontal text should be wider than tall');
    expect(vb.height, greaterThan(vb.width), reason: 'vertical text should be taller than wide');

    final outlined = await rasterizeTextObject(
      base.copyWith(outline: const TextOutline(enabled: true, color: ui.Color(0xFFFF3040), width: 3)),
      240, 160,
    );
    expect(outlined, isNotNull);
    await _saveRgba(outlined!, 240, 160, '${out.path}/text_outline.png');
    expect(_countNonTransparent(outlined), greaterThan(_countNonTransparent(horizontal)),
    ...
    expect(alphas.every((v) => v == 0 || v == 255), isTrue,
...
Brush _brush({
  ...
  id: 'audit', name: 'audit', size: size, opacity: opacity, spacing: 1,
  ...
  mixingMode: mixingMode, mixingRate: mixingRate,
);

Future<Uint8List> _drawPixelLine({required int canvas, required ui.Offset offset}) async {
  final tm = TileManager(canvasWidth: canvas, canvasHeight: canvas);
  final e = DrawingEngine(tileManager: tm)
    ..currentBrush = Brush(
      id: 'px', name: 'px', size: 7, opacity: 100, spacing: 1,
      blurRadius: 0, stabilization: false, stabilizationStrength: 0,
  ...
  for (var i = 1; i <= 24; i++) {
    final t = i / 24;
    e.continueStroke(StrokePoint(x: a.dx + (b.dx - a.dx) * t, y: a.dy + (b.dy - a.dy) * t), 'px');
  }
  e.endStroke();
...
bool _hasOpaqueNear(Uint8List d, int width, int x, int y, {required int radius}) {
  for (var yy = math.max(0, y - radius); yy <= math.min(width - 1, y + radius); yy++) {
