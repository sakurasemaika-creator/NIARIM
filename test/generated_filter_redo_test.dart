...
void main() {
  ...
  for (final kind in [
    ...
    test(
      'recorded ${kind.name} keeps neighboring layers and pixels through Redo',
      () async {
        ...
        tm.replaceLayerPixels(
          service.tileKeyFor(project.id, scene.id, 0, source.id),
          pixels,
        );
        undo.clear();
        ...
        Future<Uint8List> generatedPixels() async {
          final image = await tm.compositeLayerToImage(
            service.tileKeyFor(project.id, scene.id, 0, generatedId),
          );
          try {
