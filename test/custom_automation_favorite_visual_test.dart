...
void main() {
  ...
  testWidgets('register edit reorder rename favorite filter unfavorite delete with screenshots', (tester) async {
    ...
    service.beginDraft(name: '登録した自動操作', surface: CustomAutomationSurface.canvas, recordingStartFrame: 0);
    service.recordStep(surface: CustomAutomationSurface.canvas, command: 'canvas.tool', label: 'ペン', args: const {'tool': 'pen'}, recordedFrame: 0);
    service.recordStep(surface: CustomAutomationSurface.canvas, command: 'canvas.brushSize', label: 'サイズ', args: const {'value': 12.0}, recordedFrame: 0);
    ...
    await tester.tap(find.byKey(ValueKey('custom-automation-favorite-$itemId')));
    await tester.pumpAndSettle();
    ...
    await tester.tap(find.descendant(of: restoredRow, matching: find.byIcon(Icons.delete_outline)));
    await tester.pumpAndSettle();
