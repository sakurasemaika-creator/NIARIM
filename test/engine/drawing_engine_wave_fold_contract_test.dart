...
const foldCurve = <ui.Offset>[
  ui.Offset(60, 40), ui.Offset(180, 140),
  ui.Offset(60, 240), ui.Offset(180, 340),
];

Future<Uint8List> renderFold(HairFoldMode mode, {
  bool reverse = false, bool enabled = true,
  ...
  final engine = DrawingEngine(tileManager: tiles)
    ...
    ..currentBrush = Brush(id: 'fold', name: 'Fold', size: 44,
      spacing: 1, opacity: 100, density: 1, fadeMode: FadeMode.off,
      stabilization: false, outlineEnabled: true, outlineWidth: 2,
      outlineColor: 0xff000000, foldEnabled: enabled,
      foldTriggerAngle: 45, foldMode: mode);
  final points = reverse ? curve.reversed.toList() : curve;
  StrokePoint sample(ui.Offset p) => StrokePoint(x:p.dx,y:p.dy,
  ...
  for (var i=1;i<points.length;i++) {
    ...
    for (var j=1;j<=count;j++) {
      engine.continueStroke(sample(ui.Offset.lerp(points[i-1],points[i],j/count)!), 'test');
    }
  }
  engine.endStroke();
...
void main() {
  ...
  test('opposite views change visible fold edges, not just line thickness', () async {
  ...
  test('fold off ignores the selected mode', () async {
    expect(await renderFold(HairFoldMode.curlLeft, enabled:false),
  ...
  test('straight input is ordinary drawing for every mode', () async {
    ...
    final plain=await renderFold(HairFoldMode.waveTopView,enabled:false,curve:line);
