...
void main() {
  ...
  test('ブラシ不透明度：25/50/75%を実際に別ストロークで重ね数値どおり累積', () async {
    for (final opacity in [25, 50, 75]) {
      ...
      _line(engine, 'paint', const ui.Offset(12, 48), const ui.Offset(84, 48), steps: 24);
      ...
      _line(engine, 'paint', const ui.Offset(48, 12), const ui.Offset(48, 84), steps: 24);
      ...
      expect(singleAlpha, inInclusiveRange(expectedSingle - 4, expectedSingle + 4),
          reason: '$opacity% one stroke must preserve configured opacity');
      expect(crossAlpha, inInclusiveRange(expectedCross - 5, expectedCross + 5),
          reason: '$opacity% two separate strokes must source-over only at the intersection');
      twice.dispose();
      tm.dispose();
    }
  });
  ...
  test('筆圧不透明度：同じブラシでpressure 1.0と0.4がalphaへ比例反映', () async {
    ...
    final engine = DrawingEngine(tileManager: tm)
      ..currentBrush = _brush(size: 18, opacity: 100, pressureMode: PressureMode.opacity)
  ...
  test('手ブレ補正：同じジグザグ入力を強補正すると描画の縦振れ幅が小さくなる', () async {
    ...
    await _saveRgba(smooth, 128, 96, '${out.path}/brush_stabilization_strong.png');
    final rawSpan = _alphaYSpan(raw, 128, 96);
    final smoothSpan = _alphaYSpan(smooth, 128, 96);
    expect(smoothSpan, lessThan(rawSpan),
  ...
  test('カスタムフェード：100→20%を長い実ストロークで描くと後半ほど薄い', () async {
    ...
    final engine = DrawingEngine(tileManager: tm)
      ..currentBrush = _brush(
        ...
        fadeCustom: const FadeCustomSettings(startValue: 100, endValue: 20, distancePx: 96),
      )
  ...
  test('図形ブラシ：閉じた四角形を実描画して四辺に穴がなく内部は塗られない', () async {
    ...
    expect(_pixel(d, 96, 48, 48)[3], 0, reason: 'outline shape must not fill its interior');
    image.dispose();
    tm.dispose();
  });
  ...
  test('囲って塗る：空キャンバスでも投げ縄外へ一切漏れず選択範囲内だけ確定', () async {
    const polygon = [
      ui.Offset(15, 18), ui.Offset(79, 13), ui.Offset(85, 50),
      ui.Offset(62, 82), ui.Offset(24, 72),
    ];
    ...
    await _saveRgba(result, 96, 96, '${out.path}/lasso_enclosed_polygon_limit.png');
    var leaked = 0;
    var filled = 0;
    for (var y = 0; y < 96; y++) {
      for (var x = 0; x < 96; x++) {
...
Brush _brush({
  ...
  blurRadius: 0, stabilization: stabilization, stabilizationStrength: stabilizationStrength,
  pixelMode: false, pressureMode: pressureMode, pressureStrength: 100,
  fadeMode: fadeMode, fadeCustom: fadeCustom, strokeDecay: false,
  mixingMode: mixingMode, mixingRate: mixingRate,
);

void _line(DrawingEngine engine, String layer, ui.Offset a, ui.Offset b, {int steps = 20}) {
  engine.beginStroke(StrokePoint(x: a.dx, y: a.dy), layer);
  for (var i = 1; i <= steps; i++) {
    final t = i / steps;
    engine.continueStroke(StrokePoint(
      x: a.dx + (b.dx - a.dx) * t,
      y: a.dy + (b.dy - a.dy) * t,
    ), layer);
  }
...
Future<Uint8List> _drawJitter({required bool stabilized}) async {
  ...
  final engine = DrawingEngine(tileManager: tm)
    ...
    ..currentColor = const ui.Color(0xFF202020);
  const ys = [48.0, 31.0, 64.0, 33.0, 62.0, 35.0, 60.0, 37.0, 58.0, 40.0, 55.0, 44.0, 52.0];
