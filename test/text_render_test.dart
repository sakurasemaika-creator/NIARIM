...
void main() {
  ...
  test('縦書き・通常テキスト', () async {
    final result = await rasterizeTextObject(
      baseText('こんにちは', direction: TextWritingDirection.vertical),
      canvasWidth,
      canvasHeight,
    );
    expect(result, isNotNull);
  ...
  test('縦書き・半角英数字回転（アルファベット単独）', () async {
    final result = await rasterizeTextObject(
      baseText('ABCテスト', direction: TextWritingDirection.vertical),
      canvasWidth,
      canvasHeight,
    );
    expect(result, isNotNull);
  ...
  test('縦書き・縦中横（半角数字2桁）', () async {
    final result = await rasterizeTextObject(
      baseText('令和06年12月', direction: TextWritingDirection.vertical),
      canvasWidth,
      canvasHeight,
    );
    expect(result, isNotNull);
  ...
  test('縦書き・縦中横（3桁以上は個別回転扱いにフォールバック）', () async {
    final result = await rasterizeTextObject(
      baseText('123456', direction: TextWritingDirection.vertical),
      canvasWidth,
      canvasHeight,
    );
    expect(result, isNotNull);
  ...
  test('縦書き・整形不良のルビ記法（閉じ括弧なし）でも例外を投げない', () async {
    final result = await rasterizeTextObject(
      baseText('{閉じてない|かこ', direction: TextWritingDirection.vertical),
      canvasWidth,
      canvasHeight,
    );
    expect(result, isNotNull);
  ...
  test('空文字列（縦書き）はクラッシュせずnullを返す', () async {
    final result = await rasterizeTextObject(
      baseText('', direction: TextWritingDirection.vertical),
      canvasWidth,
      canvasHeight,
    );
  ...
  test('縦書き・ルビは実際に描画される（ルビなしより不透明ピクセルが多い）', () async {
    final withoutRuby = await rasterizeTextObject(
      baseText('明日', direction: TextWritingDirection.vertical),
      canvasWidth,
      canvasHeight,
    );
    final withRuby = await rasterizeTextObject(
      baseText('{明日|あした}', direction: TextWritingDirection.vertical),
      canvasWidth,
      canvasHeight,
    );
    expect(withoutRuby, isNotNull);
  ...
  test('縦書き・縦中横は実際に描画される（数字が正しく塗られる）', () async {
    final result = await rasterizeTextObject(
      baseText('12', direction: TextWritingDirection.vertical),
      canvasWidth,
      canvasHeight,
    );
    expect(result, isNotNull);
