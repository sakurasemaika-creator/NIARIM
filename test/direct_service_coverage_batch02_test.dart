...
void main() {
  ...
  test(
    'slot save overwrites safely, reports used slots and reloads serialized node',
    () async {
      ...
      expect(service.getUsedSlots(project.id), [2]);
      expect(
        await service.loadNode(project.id, first.id),
        isNull,
  ...
  test(
    'tree parent/child relations, archive mode change and restore preserve nodes',
    () async {
      final ps = ProjectService();
      await ps.init();
      final project = await ps.createProject(
      ...
      expect(service.getArchivedNodes(project.id).map((n) => n.id), [
        childB.id,
      ]);
      ...
      expect(
        service.getNodes(project.id).every((n) => n.slotIndex == -1),
        isTrue,
      );
      ...
      expect(
        service.getNodes(project.id).any((n) => n.id == childB.id),
        isFalse,
      );
