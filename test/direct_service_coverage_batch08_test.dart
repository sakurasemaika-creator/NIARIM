...
void main() {
  ...
  group('WatermarkService direct coverage', () {
    test('image/text add update path persistence and removal work', () async {
      ...
      await service.updateAsset(const WatermarkAsset(
        id: 'missing',
        name: 'ignored',
  ...
  group('StorageInfoService direct coverage', () {
    test('breakdown assigns live/trash/materials/exports/assets/cache exactly', () async {
      ...
      expect((await service.computeBreakdown({'trashp'}))
          .bytesOf(StorageCategory.cache), 0);
    ...
    test('eraseAllData clears NIARIM exports cache and SharedPreferences', () async {
      ...
      expect(temp.listSync(), isEmpty);
      expect((await SharedPreferences.getInstance()).getString('probe'), isNull);
    });

    test('byte formatter and DeviceSpaceInfo clamping cover unit boundaries', () {
  ...
  group('PremiumService local/non-store coverage', () {
    test('init campaign/local purchase metadata and renewal estimates persist', () async {
      ...
      expect(service.nextRenewalDateEstimate!.isAfter(DateTime.now()), isTrue);

      final restoredMonthly = PremiumService();
      await restoredMonthly.init();
      expect(restoredMonthly.hasPurchasedPremium, isTrue);
      expect(restoredMonthly.purchaseDate?.millisecondsSinceEpoch,
      ...
      expect(restoredMonthly.nextRenewalDateEstimate!.isAfter(DateTime.now()), isTrue);
