...
class Scene {
  ...
  // ユーザーが変更したシーン名（シーン名変更ダイアログ）。
  // 未設定の場合はdisplayNameがindexから自動生成する。
  final String? name;
  // カメラのXY移動・拡大・回転キーフレーム。シーンごとに管理する。
  final List<CameraKeyframe> cameraKeyframes;
  // 演出フィルター。シーンごとに管理し、プレビュー再生・書き出し時のみ適用する非破壊編集。
  final List<EffectFilterInstance> effectFilters;
  // 音声トラックのクリップ。シーンごとに管理する（音声はレイヤーを持たない）。
  final List<AudioClip> audioClips;
  // 画像・動画・音源タイムライン行のカスタム名（素材種別ごとに複数行の
  // タイムライン行を追加/削除できる。行名はタップでユーザーがテキスト
  // 変更できる）。リストの長さ＝行数（最低1）。
