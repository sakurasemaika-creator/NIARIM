...
class FilterService extends ChangeNotifier {
  ...
  static List<FilterDef> _defaultFilters() => const [
    ...
    FilterDef(id: 'Filter0023', name: '自動線画', kind: FilterKind.autoLineart, autoLineartRoughWidth: 12, autoLineartOutputWidth: 2, autoLineartTaperLength: 8, autoLineartSmoothing: 5, autoLineartColor: 0xFF000000),
  ...
  Future<void> init() async {
    ...
    if (raw == null) {
      ...
      _filters.addAll(raw.map((s) {
        ...
        if (filter.id == prismFilterId) {
          ...
          if (filter.kind != FilterKind.prism || usesLegacyDefaults) {
            filter = filter.copyWith(kind: FilterKind.prism, prismBlurPx: usesLegacyDefaults ? 17 : filter.prismBlurPx, prismDirectionDegrees: usesLegacyDefaults ? 90 : filter.prismDirectionDegrees);
            migratedAutoLineartSmoothing = true;
          }
        }
        if (filter.kind == FilterKind.autoLineart && filter.autoLineartSmoothing > 10) {
          filter = filter.copyWith(autoLineartSmoothing: (filter.autoLineartSmoothing / 10).round().clamp(0, 10).toDouble());
          migratedAutoLineartSmoothing = true;
        }
        return filter;
      }));
      ...
      if (missing.isNotEmpty || migratedAutoLineartSmoothing) await _persist();
    }
