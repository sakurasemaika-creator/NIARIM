...
class _HelpScreenState extends State<HelpScreen> {
  ...
  final List<_HelpEntry> _entries = const [
    ...
    _HelpEntry('演出フィルター', 'シーンやフレームに適用できる映像効果（ぼかし・色調補正・グロー等）'
        ...
        '調整したい時に使います。', 'アニメーション'),
    _HelpEntry('EndCard（エンドロゴ）', '動画書き出し時、本編の終わりに自動で追加されるMIRANIMAロゴの'
    ...
    _HelpEntry('共有（.mirashare）', 'プロジェクトを他の人（または自分の他の端末）へ渡すための共有専用'
        'ファイル形式です。受け取った側がこのファイルを開くと、複製されて'
        '自分のプロジェクト一覧に追加されます。共有元の.mirashare自体は'
        '変更されません。', 'プロジェクト管理'),
    _HelpEntry('引き継ぎ（.miratra）', '設定・素材・ブラシ・プリセット・UIテーマなど、アプリ全体の環境を'
        '別の端末へまとめて引き継ぐための機能です。引き継ぐ項目はチェック'
        'ボックスで個別に選べます。個別のプロジェクトを渡したい場合は'
        '「共有（.mirashare）」を使います。', 'プロジェクト管理'),
