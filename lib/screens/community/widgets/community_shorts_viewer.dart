...
/// 次/前の作品へ切り替わる。
///
/// ★**ここへ本物のYouTube埋め込みプレーヤーを入れる前に、必ず
/// `docs/AI設計書/28_継続タスク（未着手一覧）.md`の該当項目を読むこと。**
/// YouTube API利用ポリシーで次が定められており、現在のレイアウトは
/// そのままでは通らない。
/// - 自動再生は「プレーヤーの半分以上が画面に見えてから」「1画面で
///   同時に自動再生するプレーヤーは1つまで」（スクロール連動の自動再生
///   自体は許されている。ブラウザ側の都合でミュート開始が前提）。
/// - **プレーヤーの前面へオーバーレイを重ねてはならない**（許されるのは
///   同意取得と再生操作のみ）。下部220pxのグラデーション＋作品名・
///   投稿者名と右側のアクション列は、プレーヤーの外へ出す必要がある。
/// - YouTubeの中核体験（Shortsのブラウズ体験）を、独自の価値を加えずに
///   再現してはならない。NIARIM固有の価値を画面上で明確に出すこと。
